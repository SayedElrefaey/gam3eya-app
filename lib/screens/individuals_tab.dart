    ...
    final action = await showModalBottomSheet<String>(
      ...
      builder: (ctx) => SafeArea(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ListTile(leading: const Icon(Icons.edit_outlined), title: const Text('تعديل بيانات الحساب'), onTap: () => Navigator.pop(ctx, 'edit')),
          ListTile(leading: const Icon(Icons.delete_outline, color: Colors.red), title: const Text('حذف الحساب', style: TextStyle(color: Colors.red)), onTap: () => Navigator.pop(ctx, 'delete')),
      ...
      body: RefreshIndicator(
        ...
        child: _error != null
            ...
            : _loading
                ...
                : _items.isEmpty
                    ? ListView(children: [Padding(padding: const EdgeInsets.all(40), child: Text('لا يوجد ${widget.section.name} بعد\nأضف عنصر وسجل حسابه', textAlign: TextAlign.center))])
                    : ListView.builder(
                        ...
                        itemBuilder: (ctx, i) {
                          ...
                          return Container(
                            ...
                            decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), border: Border.all(color: _line), boxShadow: const [BoxShadow(color: Color(0x22000000), blurRadius: 10, offset: Offset(0, 4))]),
                            child: InkWell(
                              borderRadius: BorderRadius.circular(14),
                              onTap: () async {
                                await Navigator.push(context, MaterialPageRoute(builder: (_) => IndividualDetailScreen(id: p.id)));
                                _load();
                              },
                              ...
                              child: Padding(
                                ...
                                child: Directionality(
                                  ...
                                  child: Row(children: [
                                    Container(width: 48, height: 48, decoration: BoxDecoration(color: statusBg, borderRadius: BorderRadius.circular(14)), child: Icon(Icons.arrow_upward_rounded, color: statusColor, size: 32)),
                                    const SizedBox(width: 14),
                                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                      Text(p.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: Colors.black)),
                                      const SizedBox(height: 5),
                                      Text('${fmtNum(balance.abs())} $sym', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: statusColor)),
                                    ])),
                                    const SizedBox(width: 8),
                                    Column(children: [
                                      Container(width: 38, height: 44, decoration: BoxDecoration(color: gold, borderRadius: BorderRadius.circular(8)), alignment: Alignment.center, child: Text('${p.entries.length}', style: TextStyle(color: cover, fontSize: 17, fontWeight: FontWeight.w900))),
                                      const SizedBox(height: 4),
                                      Text('حركات', style: TextStyle(fontSize: 10, color: cover, fontWeight: FontWeight.bold)),
                                    ]),
                                  ]),
                                ),
                              ),
