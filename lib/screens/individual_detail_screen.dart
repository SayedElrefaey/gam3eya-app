...
class _IndividualDetailScreenState extends State<IndividualDetailScreen> {
        ...
        builder: (ctx, setSt) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Text(existing != null ? 'تعديل الحركة' : 'إضافة حركة على الحساب', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold), textAlign: TextAlign.right),
                const SizedBox(height: 14),
                TextField(controller: noteCtrl, textAlign: TextAlign.right, decoration: const InputDecoration(labelText: 'الوصف', border: OutlineInputBorder())),
      ...
      builder: (ctx) => SafeArea(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ListTile(leading: const Icon(Icons.print), title: const Text('طباعة / حفظ PDF'), onTap: () async { Navigator.pop(ctx); await previewInvoicePdf(bytes, 'فاتورة ${p.name}'); }),
            ListTile(leading: const Icon(Icons.share), title: const Text('إرسال عبر واتساب / مشاركة'), onTap: () async { Navigator.pop(ctx); await shareInvoiceViaWhatsApp(bytes, 'فاتورة ${p.name}', 'فاتورة ${p.name} - الرصيد: ${fmtNum(runningBalance.abs())} $sym'); }),
          ]),
        ),
      ),
    );
  ...
  Future<void> _showEntryActions(Entry e) async {
    await showModalBottomSheet(
      ...
      builder: (ctx) => SafeArea(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ListTile(leading: const Icon(Icons.edit, color: cover), title: const Text('تعديل الحركة'), onTap: () { Navigator.pop(ctx); _openEntryForm(existing: e); }),
          ListTile(leading: const Icon(Icons.delete, color: Colors.red), title: const Text('حذف الحركة'), onTap: () async {
            Navigator.pop(ctx);
            final ok = await showDialog<bool>(
              context: context,
            ...
            if (ok == true) await _deleteEntry(e.id);
          }),
        ]),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final p = _p!;
    final sym = currencySymbols[p.currency] ?? p.currency;
    final chronological = [...p.entries]..sort((a, b) {
      final da = DateTime.parse(a.entryDate);
      final db = DateTime.parse(b.entryDate);
      final cmp = da.compareTo(db);
      return cmp != 0 ? cmp : a.id.compareTo(b.id);
    ...
    for (final e in chronological) {
      if (e.type == 'debit') { running += e.amount; totalDebit += e.amount; }
      else { running -= e.amount; totalCredit += e.amount; }
    ...
    return Directionality(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(backgroundColor: cover, foregroundColor: Colors.white, title: Text(p.name, style: const TextStyle(fontWeight: FontWeight.w500)), elevation: 2, actions: [
          IconButton(icon: const Icon(Icons.search), onPressed: () {}),
          PopupMenuButton<String>(onSelected: (value) { if (value == 'invoice') _sendInvoice(); if (value == 'delete') _delete(); }, itemBuilder: (_) => const [PopupMenuItem(value: 'invoice', child: Text('فاتورة PDF')), PopupMenuItem(value: 'delete', child: Text('حذف الفرد'))]),
        ]),
          ...
          child: ListView(
            ...
            children: [
              ...
              Row(children: [
                ...
                const SizedBox(width: 10),
                Expanded(child: OutlinedButton.icon(onPressed: _sendInvoice, icon: const Icon(Icons.share), label: const Text('إرسال واتساب'))),
              ]),
              const SizedBox(height: 10),
        ...
        floatingActionButton: FloatingActionButton(onPressed: () => _openEntryForm(), backgroundColor: gold, foregroundColor: cover, child: const Icon(Icons.note_add)),
      ),
    );
  }

  Widget _quickAction(IconData icon, String label, VoidCallback onTap) => Expanded(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 3), child: Column(children: [
    InkWell(borderRadius: BorderRadius.circular(18), onTap: onTap, child: Container(width: 58, height: 58, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: _line)), child: Icon(icon, color: gold, size: 34))),
  ...
  Widget _summaryBox(double totalDebit, double totalCredit, double balance, String sym) => Container(
    ...
    child: Column(children: [
      ...
      Text('الرصيد ${balance >= 0 ? 'عليه' : 'له'}: ${fmtNum(balance.abs())} $sym'.trim(), style: const TextStyle(fontSize: 18, color: cover, fontWeight: FontWeight.w800)),
    ]),
  );
