...
class _Gam3eyasTabState extends State<Gam3eyasTab> {
            ...
            child: SingleChildScrollView(
              child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                ...
                DropdownButtonFormField<int>(
                  ...
                  items: const [DropdownMenuItem(value: 1, child: Text('شهري')), DropdownMenuItem(value: 3, child: Text('كل 3 شهور')), DropdownMenuItem(value: 6, child: Text('كل 6 شهور'))],
                ...
                ListTile(
                  ...
                  title: Text(startDate == null ? 'اختر تاريخ البداية' : '${startDate!.day}/${startDate!.month}/${startDate!.year}', textAlign: TextAlign.right, style: const TextStyle(color: cover, fontWeight: FontWeight.w600)),
                  trailing: const Icon(Icons.calendar_today, color: gold),
                  onTap: () async {
                    final d = await showDatePicker(context: ctx, initialDate: startDate ?? DateTime.now(), firstDate: DateTime(2020), lastDate: DateTime(2100));
                ...
                if (widget.section.hasTurns && months > 0) ...[
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(color: _turnLight, borderRadius: BorderRadius.circular(10), border: Border.all(color: _turnBlue)),
                ...
                Row(children: [
                  ...
                  Expanded(child: ElevatedButton(
                    ...
                    onPressed: () async {
                      ...
                      if (nameCtrl.text.trim().isEmpty || startDate == null || m < 1 || amount <= 0) { setSt(() => error = 'من فضلك أكمل جميع الحقول بشكل صحيح'); return; }
                      try {
  ...
  Future<void> _rename(Gam3eya g) async {
    ...
    final res = await showDialog<Map<String, String>>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          TextField(controller: ctrl, textAlign: TextAlign.right, decoration: const InputDecoration(labelText: 'اسم الجمعية')),
          const SizedBox(height: 10),
          TextField(controller: sortCtrl, textAlign: TextAlign.right, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'الترتيب في القائمة')),
        ]),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('إلغاء')),
          ElevatedButton(onPressed: () => Navigator.pop(ctx, {'name': ctrl.text.trim(), 'sort': sortCtrl.text.trim()}), child: const Text('حفظ')),
        ],
      ),
    );
    ctrl.dispose();
    ...
    if (res != null && (res['name'] ?? '').isNotEmpty) {
      try {
        await ApiService.renameGam3eya(g.id, res['name']!, sortOrder: int.tryParse(res['sort'] ?? '') ?? 0);
        _load();
  ...
  Future<void> _delete(Gam3eya g) async {
    ...
    if (ok == true) {
      try { await ApiService.deleteGam3eya(g.id); _load(); }
      catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString()))); }
    }
  }
                        ...
                        itemBuilder: (ctx, i) {
                          ...
                          return Card(
                            ...
                            child: InkWell(
                              ...
                              onTap: () async { await Navigator.push(context, MaterialPageRoute(builder: (_) => Gam3eyaDetailScreen(id: g.id))); _load(); },
                              child: Padding(
                                ...
                                child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                                  ...
                                  if (widget.section.hasTurns) ...[
                                    ...
                                    Container(
                                      ...
                                      child: Row(children: [
                                        const Icon(Icons.star, size: 18, color: _turnBlue), const SizedBox(width: 6),
  ...
  Widget _infoBox(String label, String value) {
    return Container(
      ...
      child: Column(children: [Text(label, style: const TextStyle(fontSize: 11, color: Color(0xFF6B6248))), const SizedBox(height: 3), Text(value, textAlign: TextAlign.center, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800, color: cover))]),
    );
  }
}
